list.of.packages <- c("glcm","raster","spatial.tools","dplyr","doParallel")
...
rasterFileName <- "geomatica_SPOT_panshp"
...
window_sizes <- c(3,5,9) # specifies the size of the window to use in glcm
...
glcm_layers <- create_GLCM_layers_parallel(list_rasterlayers = list_layersforGLCM,
                                           ...
                                           dir = diag_dir,
                                           cpus = cpus)
glcm_stack_diag <- do.call("stack",glcm_layers)
...
r <- stack(raster_stack,NDVI,SAVI,ratio1,ratio2,ratio3,ratio4,glcm_stack_all, glcm_stack_vert, glcm_stack_diag)
